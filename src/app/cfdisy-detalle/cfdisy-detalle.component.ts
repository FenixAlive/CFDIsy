...
export class CfdisyDetalleComponent implements OnInit {
  ...
  showConcep(isIt: boolean): void {
    if (isIt) {
      this.conceptos.nativeElement.innerHTML = `
  ...
  addConcep(
    ...
    const data = [unidad, claveUnidad, claveProdServ, numIde];
    const tit = [
      'Unidad',
      'Clave Unidad',
    ...
    <div class="cabCols" id="concepto_${idx}">
      <div class="cabrows borde prim1 padUD" id="concepBase_${idx}">
        ${this.concepPrinData(cantidad, valorUni, descrip, descuento, importe)}
      </div>
  ...
  trasRetConcepCabe(idx: any) {
    @ViewChild(`datosAd_${idx}`) temp!: ElementRef;
    temp.nativeElement.insertAdjacentHTML()
  ...
  addTrasRetConcep(idx, id, tipo, importe, tasa, tipoFactor, impuesto, base) {
    var name = '';
    if (tipo == 'Traslados') {
      name = 'Traslado';
    ...
    document
      .getElementById(`trasRetConcep_${idx}_${id - 1}`)
      .insertAdjacentHTML(
        'afterend',
  ...
  addTrasRetImp(idx, tipo, impuesto, factor, tasa, importe) {
    if (tipo == 'Retencion') {
      importe = '- ' + importe;
    }
    if (factor == 'Tasa') {
  ...
  rellenar(id, titulo, data, borde) {
    if (borde) {
      reInfoTitData(
        id,
        'cabrowsStart flexGrow1 borde',
        'textAlL',
        titulo,
        data
      );
  ...
  rellenar_cortado(id, titulo, data) {
  ...
  pagoCabe(id, fechaP, formaPP, monedaP, montoP, otros) {
  ...
  addTrasRetPago(idP, idx, tipo, impuesto, factor, tasa, importe) {
    if (idx == 0) {
  ...
  infoAduCuentaPred(idx, nombre, titulo, data) {
    document.getElementById(`datosAd_${idx}`).insertAdjacentHTML(
      'afterend',
  ...
  elementoClickeable(idx, idCabe, idDet, htmlAgreg, idBase, color1, color2) {
    document
  ...
  soloClicker(idBase, idDet, color1, color2) {
  ...
  reInfoTitData(id, clase, lado, titulo, data) {
    if (data != '') {
  ...
  cabezaNodo(nom_p, id_padre, el_name, titulo, capa, id_nodo, id_acomodo) {
    var color1, color2;
    if (capa % 2 == 0) {
      color1 = 'prim1';
      color2 = 'prim2';
    ...
    if (capa == 0) {
      ...
      document.getElementById(`${nom_p}_${id_padre}_cuerpo`).innerHTML =
        contenedor;
    } else {
      document
  ...
  atributosNodo(el_name, capa, id_nodo, titAtt, att) {
    var atributos = '';
    ...
    document.getElementById(`${el_name}_${id_nodo}_cuerpo`).innerHTML =
      atributosCont;
  }
  */
}
